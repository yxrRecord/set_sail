...
util.install = function (Vue) {
  ...
  Vue.prototype.$ajaxPost = (requestOptions, showLoading, callback) => {
    ...
    if (requestUrl.substring(requestUrl.length - 1) === "?") {
      ...
      if (requestOptions.data.refresh) delete requestOptions.data.refresh;
    ...
    if (
      ...
      Object.keys(ajaxOptions.data).length > 0
    ) {
      ajaxOptions.url += "?";
  ...
  Vue.prototype.$validate = new (function () {
    ...
    this.methods = {
      required: (value) => {
        if (Vue.prototype.$getType(value) === "number") return true;
        if (value) if (value === 0 || value === false) return true;
        if (!value) return false;
        let length = Array.prototype.isPrototypeOf(value)
          ? value.length
      ...
      identityNums: (value) => {
        if (value === "" || value === null) return true;
        if (
          (value.length < 18 && value.length > 15) ||
          value.length < 15 ||
          value.length > 18
        ) {
          return false;
  ...
  Vue.prototype.$downLoad = (options, encode) => {
    ...
    if (encode) ajaxOptions.url = encodeURI(ajaxOptions.url);
  ...
  Vue.prototype.$mergeTableByColData = (tableData, key, childKey) => {
    ...
    tableData.forEach((item, index) => {
      if (index === 0) {
        ...
        if (childKey) {
          if (item[childKey][key] === tableData[index - 1][childKey][key]) {
            //第一列需合并相同内容的判断条件
            spanArr[concatOne] += 1;
            spanArr.push(0);
          } else {
